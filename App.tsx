...
import { 
  X, Plus, Hand, Download, Eye, Sparkles, Activity, Settings, Type as TypeIcon, Palette, Sliders, MessageSquare, Zap, CreditCard, CheckCircle2
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // ... 之前的状态 (textConfig, analysis 等) 保持不变 ...
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col items-center justify-center overflow-hidden bg-[#FDFCFB]">
      ...
      <header className="fixed top-0 w-full h-24 z-[70] px-12 flex items-center justify-between border-b border-stone-50 bg-white/40 backdrop-blur-md">
        ...
        <button onClick={startPay} className="flex items-center gap-3 px-6 py-3 bg-white border border-stone-100 rounded-full hover:shadow-xl transition-all group">
          <CreditCard className="w-4 h-4 text-[#002FA7]" />
          <span className="text-[10px] font-black tracking-widest uppercase">升级 Pro 版</span>
        </button>
      </header>
      ...
      <main className="relative z-30 w-full max-w-6xl px-6 py-32 flex flex-col items-center">
        ...
        {step === 'upload' ? (
               ...
               <div className="lg:col-span-7 glass-panel p-12 space-y-12 rounded-[60px]">
                ...
                <section>
                  ...
                  <textarea 
                    ...
                    className="w-full min-h-[120px] resize-none rounded-sm bg-white/40 backdrop-blur-md border border-stone-200/60 px-4 py-3 text-sm text-stone-800 focus:bg-white/80 transition-all outline-none"
                ...
                <div className="glass-panel p-10 space-y-12 rounded-[48px]">
                   <section>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {SCENARIO_CONFIGS.slice(0, 4).map(s => (
                        <button key={s.id} onClick={() => setSelectedScenario(s.id)} className={`p-4 border rounded-[24px] text-left ${selectedScenario === s.id ? 'border-[#002FA7] bg-[#002FA7]/5' : 'bg-white/50'}`}>
                  ...
                  <section>
                    ...
                    <div className="flex gap-2 p-1.5 bg-stone-50 rounded-[20px]">
                      <button onClick={() => setMode('precision')} className={`flex-1 py-4 text-[10px] font-black rounded-2xl ${mode === 'precision' ? 'bg-white text-[#002FA7]' : 'text-stone-300'}`}>物理保真</button>
                      <button onClick={() => setMode('creative')} className={`flex-1 py-4 text-[10px] font-black rounded-2xl ${mode === 'creative' ? 'bg-white text-[#002FA7]' : 'text-stone-300'}`}>艺术重构</button>
                    </div>
                  </section>
                </div>
                <button onClick={handleGenerate} className="w-full h-24 bg-orange-500 text-white rounded-[32px] font-black tracking-[1em] text-[13px] shadow-2xl transition-all active:scale-95">启动渲染引擎</button>
              </div>
            </div>
          </div>
        ) : (
          ...
          <div className="w-full max-w-5xl reveal-up">
            {isProcessing ? (
              <div className="flex flex-col items-center py-40">加载中...</div>
            ) : (
              <div className="space-y-16 text-center">
                <img src={resultImage!} className="w-full h-auto rounded-[40px] shadow-2xl" alt="" />
                <button onClick={() => setStep('upload')} className="px-10 py-5 bg-white border rounded-2xl text-[11px] font-black uppercase">返回重试</button>
              </div>
            )}
          </div>
        )}
      ...
      {showCheckout && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
          <div className="absolute inset-0 bg-stone-900/80 backdrop-blur-2xl" onClick={() => setShowCheckout(false)} />
          <div className="relative w-full max-w-md bg-white rounded-[48px] p-12 text-center shadow-2xl reveal-up">
            <button onClick={() => setShowCheckout(false)} className="absolute top-8 right-8 p-3 bg-stone-50 rounded-full"><X size={20}/></button>
            ...
            <div className="space-y-8">
              <div className="flex flex-col items-center gap-2">
                ...
                <h4 className="text-2xl font-serif italic font-black">解锁 Pro 无限算力</h4>
                <p className="text-[11px] text-stone-400 tracking-[0.2em] font-black uppercase">Gemini 2.5 Flash Ultra Powered</p>
              </div>

              <div className="bg-stone-50 rounded-[32px] p-10 flex flex-col items-center gap-6 border border-stone-100 shadow-inner">
                {qrUrl ? (
                  <div className="relative p-4 bg-white rounded-2xl shadow-xl">
                    <img src={qrUrl} className="w-56 h-56" alt="支付二维码" />
                    {payStatus === 'success' && (
                      <div className="absolute inset-0 bg-white/90 flex flex-col items-center justify-center gap-4 rounded-2xl">
                        <div className="w-16 h-16 bg-green-500 text-white rounded-full flex items-center justify-center animate-bounce">
                          <CheckCircle2 size={40} />
                        </div>
                        <span className="font-black text-green-600">支付成功</span>
                      </div>
                    )}
                  </div>
                ) : (
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-[10px] font-black text-[#002FA7] tracking-widest uppercase">支持支付宝扫码</div>
                </div>
              </div>

              <p className="text-[10px] text-stone-300 leading-relaxed px-4">
                点击扫码支付即代表您同意《电商宝商业授权协议》，支付完成后系统将自动为您分配独立神经元计算节点。
              </p>
            </div>
          </div>
        </div>
