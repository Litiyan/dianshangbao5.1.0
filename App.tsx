  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col items-center justify-center overflow-hidden bg-[#FDFCFB]">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none flex gap-10 px-10">
        ...
        <div className="hidden lg:flex flex-1 flex flex-col gap-10 bg-flow-up opacity-[0.05]">
          {[...BG_IMAGES, ...BG_IMAGES].map((src, i) => <img key={i} src={src} className="w-full aspect-[4/5] object-cover rounded-[40px] grayscale" alt="" />)}
        </div>
      ...
      <div className="fixed inset-0 z-10 flex items-center justify-center pointer-events-none select-none">
        <h2 className="text-klein-gradient text-[16vw] font-serif italic font-black tracking-tighter opacity-[0.07] uppercase whitespace-nowrap">
          REDEFINE
        </h2>
      </div>
      ...
      <header className="fixed top-0 w-full h-24 z-[70] px-12 flex items-center justify-between border-b border-stone-50 bg-white/40 backdrop-blur-md">
        <div className="flex items-center gap-4 group cursor-pointer">
          <div className="relative p-2 bg-orange-50 rounded-2xl group-hover:scale-110 transition-transform duration-500">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[9px] font-sans font-black text-orange-400 tracking-[0.3em] mt-1.5 uppercase opacity-60">Pure Vision Pro</span>
          </div>
        </div>
        ...
        {step === 'upload' ? (
          <div className="w-full space-y-24 reveal-up">
            <div className="text-center space-y-8">
              ...
              <p className="font-sans text-[12px] text-stone-400 tracking-[1.2em] font-black uppercase inline-block border-y border-stone-100 py-4">
                画 廊 级 的 影 像 叙 事 重 构
              </p>
            </div>
            ...
            <div className="grid lg:grid-cols-12 gap-12">
              {/* 左侧工作区 */}
              <div className="lg:col-span-7 glass-panel p-12 space-y-16 rounded-[60px]">
                <section>
                  <label className="text-[11px] font-black text-[#002FA7] tracking-[0.3em] uppercase mb-10 flex items-center gap-4">
                    <span className="w-2.5 h-2.5 bg-[#002FA7] rounded-full shadow-[0_0_15px_rgba(0,47,167,0.4)]" />
                    [ 01 ] 核心锚点注入
                  </label>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
                    {sourceImages.map((img, i) => (
                      <div key={i} className="aspect-square relative group bg-white rounded-[24px] overflow-hidden shadow-sm border border-stone-50">
                    ...
                    {sourceImages.length < 5 && (
                      <label className="aspect-square border-2 border-dashed border-stone-100 rounded-[24px] flex flex-col items-center justify-center cursor-pointer hover:border-[#002FA7] hover:bg-white transition-all group">
                        ...
                        <Plus className="text-stone-200 group-hover:text-[#002FA7] transition-colors" size={32} />
                ...
                <section>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {SCENARIO_CONFIGS.map(s => (
                      <button key={s.id} onClick={() => setSelectedScenario(s.id)} className={`flex items-start gap-5 p-5 border rounded-[28px] transition-all text-left ${selectedScenario === s.id ? 'border-[#002FA7] bg-[#002FA7]/5' : 'border-stone-50 bg-white/50 hover:border-stone-200'}`}>
                        <span className="text-xl">{s.icon}</span>
                        <div>
                          <div className={`text-[13px] font-black ${selectedScenario === s.id ? 'text-[#002FA7]' : 'text-stone-800'}`}>{s.name}</div>
                ...
                <div className="glass-panel p-10 space-y-12 rounded-[48px]">
                  <section>
                    ...
                    <div className="space-y-6">
                      <input type="text" placeholder="主标题文案" value={textConfig.title} onChange={e => setTextConfig({...textConfig, title: e.target.value})} className="input-gallery w-full font-serif text-xl" />
                      <div className="grid grid-cols-2 gap-3">
                        {[
                          { id: 'modern', label: '现代促销', css: 'font-sans font-black' },
                          { id: 'elegant', label: '优雅轻奢', css: 'font-serif italic font-bold' },
                          { id: 'calligraphy', label: '东方书法', css: 'font-serif font-light' },
                          { id: 'playful', label: '活力种草', css: 'font-sans font-bold' }
                        ].map(style => (
                          <button key={style.id} onClick={() => setTextConfig({...textConfig, fontStyle: style.id as FontStyle})} className={`py-4 text-[11px] rounded-[18px] border transition-all ${textConfig.fontStyle === style.id ? 'bg-[#002FA7] text-white border-[#002FA7] shadow-lg' : 'text-stone-400 border-stone-50 bg-white/40'} ${style.css}`}>{style.label}</button>
                        ))}
                      </div>
                    </div>
                  </section>
            ...
            {isProcessing ? (
              <div className="flex flex-col items-center py-40 space-y-16">
                <div className="relative w-56 h-56 flex items-center justify-center">
                  ...
                  <div className="absolute inset-8 border border-orange-500/10 rounded-full animate-spin duration-[10000ms]" />
                  <div className="w-16 h-16 bg-white shadow-2xl rounded-full flex items-center justify-center">
                    <Hand className="text-orange-500 animate-bounce" size={32} />
                  </div>
                </div>
                <div className="text-center space-y-4">
                  <h4 className="text-4xl font-serif italic text-stone-900">正在构建您的视觉资产...</h4>
                  <p className="text-[10px] font-black text-stone-200 tracking-[0.5em] uppercase">Architecture Rendering Engine In Progress</p>
                </div>
              </div>
            ) : (
              <div className="space-y-16">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-10 border-b border-stone-100 pb-16">
                  <div className="space-y-4">
                    <h2 className="text-6xl font-serif italic text-stone-900 tracking-tighter leading-none">The Masterpiece<span className="text-[#002FA7]">.</span></h2>
                ...
                <div className="relative group glass-panel p-8 rounded-[64px]">
                   ...
                   <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-1000">
                     <div className="w-20 h-20 bg-white/90 backdrop-blur-xl rounded-full flex items-center justify-center shadow-2xl scale-90 group-hover:scale-100 transition-transform"><Eye className="text-orange-500" size={32} /></div>
                   </div>
                </div>
              </div>
            )}
      ...
      <footer className="fixed bottom-0 w-full h-16 border-t border-stone-50 bg-white/60 backdrop-blur-xl flex items-center justify-center px-12 z-[80]">
        <div className="text-[10px] text-stone-200 tracking-[1.5em] font-serif uppercase">© 2025 // PURE GALLERY // KLEIN BLUE EDITION</div>
      </footer>

      {error && (
        <div className="fixed bottom-24 right-10 glass-panel p-10 z-[100] border-orange-500/20 max-w-sm rounded-[32px] reveal-up">
           <div className="flex items-center gap-4 text-orange-600 mb-4">
             <Activity size={20} />
             <span className="text-[12px] font-black tracking-widest uppercase">核心引擎中断</span>
           </div>
           <p className="text-[13px] text-stone-500 leading-relaxed">{error}</p>
           <button onClick={() => setError(null)} className="mt-6 text-[11px] text-stone-300 hover:text-orange-500 underline underline-offset-8 transition-colors">关闭提示</button>
        </div>
      )}
    </div>
  );
